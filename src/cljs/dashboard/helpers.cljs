...
(defn group-dispatcher-and-command
  ...
  (reduce (fn [commands-to-run [section-id section]]
            (apply conj commands-to-run (map (fn [[command-id command]]
                                               (identity [(keyword dispatcher)
                                                          {:section section-id :command command-id}]))
                                             (:commands section))))
          []
          sections->commands))
