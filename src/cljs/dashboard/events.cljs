...
(defn redirect
  [uri]
  (set! js/location.hash (str "/" (clj->js uri))))
...
(reg-event-fx
  ...
  (fn [{:keys [db]} _]
    {:dispatch [:set-active-page :home]
     :db       (do
                 (redirect :home)
                 (assoc-in db [:user :authenticated?] true))}))
...
(reg-event-fx
  ...
  (fn [{:keys [db]} _]
    {:dispatch [:set-active-page :home]
     :db       (do
                 (set! js/location.hash "/home")
                 (assoc-in db [:user :authenticated?] false))}))
...
(reg-event-db
  :set-admin-command-result
  trim-v
  (fn [db [{:keys [section command result]} :as section->command]]
    (assoc-in db [:advanced :admin-command-results] {:command command :result
                                                              (clojure.string/replace result #"\n" "")})))

(reg-event-db
  :clear-admin-command-results
  trim-v
  (fn [db _]
    (update-in db [:advanced] dissoc :admin-command-results)))
...
(reg-event-fx
  ...
  trim-v
  (fn [{:keys [db]} _]
    (send-post-request ["/advanced/run" {:section "Admin" :command "Shutdown"} :set-admin-command-result])))

(reg-event-fx
  :send-configure-wan-request
  trim-v
  (fn [{:keys [db]} _]
    (send-post-request ["/advanced/run" {:section "Admin" :command "Gigabit"} :set-admin-command-result])))

(reg-event-fx
  :send-initiate-upload-request
  trim-v
  (fn [{:keys [db]} _]
    (send-post-request ["/advanced/run" {:section "Admin" :command "Upload"} :set-admin-command-result])))
