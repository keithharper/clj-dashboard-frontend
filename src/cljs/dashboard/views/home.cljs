...
(defn alert-container []
  (let [alert-message @(subscribe [:alert-message])]
    (when-not (nil? alert-message)
      [:div.nav-item.command-alert.active alert-message])))
...
(defn command-nodes-to-section [{:keys [section command result] :as section->command}]
  ...
  [:div.card__command-node
   [:div.card__command-status
    {:on-click #(dispatch [:fetch-dashboard-command-result section->command])}
    (parse-result result)]
  ...
  [:div.section.col-md-4
   [:div.card__container
    [:div.card__command-section
     ...
     (map
       #(command-nodes-to-section (conj {:section section-id :result (:result (second %))} (second %)))
       (:commands section))]]])

(defn command-nodes []
  (when-let [commands @(subscribe [:available-dashboard-commands])]
    (map
      #(base-section-node {:section-id (first %) :section (second %)})
      commands)))
